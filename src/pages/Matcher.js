...
export class MatcherPage extends React.Component {
    ...
    fillArray = () => {
        ...
        const male = _.filter(this.state.users, p => p.sex == 1 && p.status == 0).sort();
        const female = _.filter(this.state.users, p => p.sex == 0 && p.status == 0).sort();
        this.setState({ indexMale: -1, indexFemale: -1, male, female }, () => {  });
        console.log("fillArray", this.state.male, this.state.female)
    }
    ...
    handleChangeFemale = (event) => {
        ...
        console.log("handleChangeFemale", this.state.indexFemale);
    }
    ...
    handleChangeMale = (event) => {
        ...
        console.log("11111111111indexMale", this.state.users[this.state.indexMale]);

        console.log("handleChangeMale", this.state.indexMale);
    }
    ...
    makeMatch = () => {
        console.log("makeMatch male", this.state.male)
        console.log("makeMatch female", this.state.female)
        console.log("user ----1----", this.state.users[this.state.indexFemale], this.state.users[this.state.indexMale]);
        if (this.state.indexFemale != -1 && this.state.indexMale != -1) {
            ...
            this.setState({ users })
            console.log("aaaaaaaaaaaaa", this.state.users);
